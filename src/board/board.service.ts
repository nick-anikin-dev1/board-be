...
export class BoardService {
  ...
  async update (dto: UpdateBoardDto, user: IUser) {
    const existUser = await this.existUser(user)
    const project = await this.projectService.findOneBy({where: {
    ...
    const renameBoard = await this.boardRepository.save(project.boards.filter((board) => {
      if (board.name === dto.oldName)
        return board.name = dto.newName
    }))
    if (!renameBoard[0]) {
  ...
  async existUser(user: IUser) {
    return await this.userService.findOneBy({where: {
      id: user.id
    },
